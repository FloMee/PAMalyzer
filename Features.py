...
class Features:
    ...
    def get_Raven_robust_measurements(self, f1, f2):
        ...
        return (
            freqindices,
            freqindices[3] - freqindices[1],
            freqindices[4] - freqindices[0],
            timeindices,
            timeindices[3] - timeindices[1],
            timeindices[4] - timeindices[0],
        )
...
def testFeatures():
    wavobj = wavio.read(
        "D:\AviaNZ\Sound_Files\Denoising_paper_data\Primary_dataset\kiwi\male\male1.wav"
    )
    fs = wavobj.rate
    data = wavobj.data
    ...
    for root, dirs, files in os.walk(str(dir_src)):
        for file in files:
            if (file.endswith(".wav") and os.stat(root + "/" + file).st_size
                ...
                if "WE" in feature:
                    currWCs = ws.computeWaveletEnergy(
                        data=data,
