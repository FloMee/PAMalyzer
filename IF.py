...
class IF:
    ...
    def ecurve(self, TFR, freq, wopt):
        ...
        if np.amin(freq) <= 0 or np.std(np.diff(freq, 1, 0)) < np.std(
            np.diff(np.log(freq), 1, 0)
        ):
            fres = 1
        ...
        if isinstance(wopt, Wopt):
            ...
            if self.method == 1 or self.method == 3:
                ...
                # time
                f1 = (
                    lambda t: np.sin((2 * np.pi * t) / wopt[1])
                    - (2 * np.pi * t) / wopt[1]
                    + np.pi / 2
                )
        ...
        if (
            isinstance(self.method, str) and self.method.lower() != "max"
        ) or isinstance(self.method, int):
            sflag = 1
            # ----------------------------------------------------------------------
            ...
            if self.Skel:
                ...
                TFR = np.vstack(
                    (np.zeros((1, L)), TFR, np.zeros((1, L)))
                )  # pad TFR with zeros

                # find indices of the peaks
                idft = 1 + np.argwhere(
                    (TFR.flatten("F")[1:-1] >= TFR.flatten("F")[0:-2])
                    & (TFR.flatten("F")[1:-1] > TFR.flatten("F")[2:])
                )  # flatten TFR
                ...
                for dn in range(len(dind) - 1):
                    ii = np.arange(
                        dind[dn] + 1, dind[dn + 1] + 1, 1
                    )  # ii are indeces first +1 not needed
                    idn[ii] = np.reshape(
                        np.arange(0, len(ii), 1), np.shape(idn[ii])
                    )  # changef 1 in 0
                ...
                Indeces_peaks = np.reshape(
                    Indeces_peaks_copy, np.shape(Indeces_peaks), "F"
                )
                del Indeces_peaks_copy
                Amplitude_peaks_copy = Amplitude_peaks.flatten("F")
                Amplitude_peaks_copy[idnt] = a2 - (1 / 4) * (a1 - a3) * dp
                Amplitude_peaks = np.reshape(
                    Amplitude_peaks_copy, np.shape(Amplitude_peaks), "F"
                )
                del Amplitude_peaks_copy
                if fres == 1:
                    Frequency_peaks_copy = Frequency_peaks.flatten("F")
                    Frequency_peaks_copy[idnt] = (
                        np.reshape(freq[idf], (np.shape(idf)[1])) + dp[:] * fstep
                    )
                    Frequency_peaks = np.reshape(
                        Frequency_peaks_copy, np.shape(Frequency_peaks), "F"
                    )
                ...
                idb = np.argwhere(
                    (np.isnan(dp)) | (np.abs(dp) > 1) | (idf == 0) | (idf == NF - 1)
                )
                ...
                if self.DispMode.lower() != "off":
                    if self.DispMode.lower() != "notify":
                        print(
                            "(number of ridges:",
                            self.Round(np.mean(Number_peaks[tn1 : tn2 + 1])),
                            " +- ",
                            self.Round(np.std(Number_peaks[tn1 : tn2 + 1])),
                            " from ",
                            np.amin(Number_peaks),
                            " to ",
                            np.amax(Number_peaks),
                            ")\n",
                        )
                    ...
                    if NB > 0:
                        print(
                            "Warning: At ",
                            NB,
                            " times there are no peaks (using border points instead).\n",
                        )
            ...
            isinstance(self.method, str) and len(self.method) > 3
        ):  # frequency-based extraction
            if len(self.method) != L:
                print(
                    'The specified frequency profile ("Method" property) should be of the same length as signal.'
                )
            ...
            if self.DispMode.lower() != "off" and self.DispMode.lower() != "notify":
                if submethod == 1:
                    print(
                        "Extracting the ridge curve lying in the same TFR supports as the specified frequency profile.\n"
                    )
                else:
                    print(
                        "Extracting the ridge curve lying nearest to the specified frequency profile.\n"
                    )

            tn1 = np.amax(
                [tn1, np.flatnonzero(np.isnan(efreq) == False)[0]]
            )  # we want just the first one
            ...
            for tn in range(tn1, tn2 + 1):
                ...
                if cind > 1 and cind < NF:
                    if cs[cind + 1] == cs[cind - 1] or submethod == 2:
                        cpeak1 = (
                            ...
                            - 1
                            + np.flatnonzero(
                                cs[cind:-1] >= cs[cind - 1 : -2]
                                and cs[cind:-1] > cs[cind + 1 :]
                            )[0]
                        )  # check
                        ...
                        cpeak2 = (
                            ...
                            + 1
                            - np.flatnonzero(
                                cs[cind:1:-1] >= cs[cind + 1 : 2 : -1]
                                and cs[cind:1:-1] > cs[cind - 1 :: -1]
                            )[0]
                        )  # check
                        ...
                        cpeak = (
                            ...
                            + 1
                            - np.flatnonzero(
                                cs[cind:0:-1] > cs[cind - 1 :: -1]
                                and cs[cind:0:-1] >= cs[cind + 1 : 1 : -1]
                            )[0]
                        )  # check
                    ...
                    if cs[1] < cs[0]:
                        ...
                        cpeak = (
                            1
                            + np.flatnonzero(
                                cs[cind + 1 : -1] >= cs[cind:-2]
                                and cs[cind + 1 : -1] > cs[cind + 2 :]
                            )[0]
                        )  # check
                        cpeak = np.amin([cpeak, NF])
                elif cind == NF:
                ...
                if cpeak < NF - 1:
                    iup = (
                        cpeak
                        + np.flatnonzero(
                            cs[cpeak + 1 : -1] <= cs[cpeak:-2]
                            and cs[cpeak + 1 : -1] < cs[cpeak + 2 :]
                        )[0]
                    )
                ...
                if cpeak > 2:
                    idown = (
                        cpeak
                        - np.flatnonzero(
                            cs[cpeak - 1 : 0 : -1] <= cs[cpeak:1:-1]
                            and cs[cpeak - 1 : 0 : -1] < cs[cpeak - 2 :: -1]
                        )[0]
                    )  # check
                iup = np.amin([iup, NF])
            ...
            p_amplitude[tn1 : tn2 + 1] = np.abs(
                TFR[
                    np.ravel_multi_index(
                        [p_index[tn1 : tn2 + 1], np.arange(tn1, tn2 + 1)], np.shape(TFR)
                    )
                ]
            )  # check
        ...
        if (type(self.method) == "char" and self.method.lower() == "max") or len(
            self.pars
        ) == 2:
            if self.DispMode.lower() != "off" and self.DispMode != "notify":
                if sflag == 0:
                    ...
                    print(
                        "Extracting positions of global maximums (needed to estimate initial parameters).\n"
                    )
            ...
            if sflag == 0:
                if self.NormMode.lower() == "on":
                    ...
                    TFR = TFR * (
                        nfunc[:] @ np.ones((1, L))
                    )  # this should be a vector moltiplication (not element-wise)
                ...
                if self.NormMode.lower() == "on":
                    ...
                    p_amplitude[tn1 : tn2 + 1] = p_amplitude[tn1 : tn2 + 1] / (
                        nfunc[p_index[tn1 : tn2 + 1]].T
                    )
            else:
                ...
                # sanitycheck
                idr = idr.astype("int")
                lid = np.ravel_multi_index(
                    [idr[tn1 : tn2 + 1], np.arange(tn1, tn2 + 1)],
                    np.shape(Frequency_peaks),
                    order="F",
                )
            ...
            idz = np.where(
                (p_amplitude[tn1 : tn2 + 1] == 0)
                | (np.isnan(p_amplitude[tn1 : tn2 + 1]))
            )  # -1 seems to be not needed
            if np.shape(idz)[1] != 0:
                idz = idz[0]
                idz += tn1 - 1
                idnz = np.arange(tn1, tn2)  # +1 omitted
                idnz = idnz[
                    np.argwhere(np.isin(idnz, idz) == False)
                ]  # =>np.in1d #this is problematic!
                p_index[idz] = np.interp(
                    idz, idnz, p_index[idnz]
                )  #'linear' in the function no equivalent of 'extrap'
                p_index[idz] = self.Round(p_index[idz])
                tfsupp[0][idz] = np.interp(
                    idz, idnz, tfsupp[0][idnz]
                )  #'linear' in the function no equivalent of 'extrap'
        ...
        if (type(self.method) == "char") and self.method.lower() == "nearest":
            ...
            if self.DispMode.lower() != "off" and self.DispMode.lower() != "notify":
                ...
                print(
                    "The highest peak was found at time ",
                    (timax - 1) / fs,
                    " s and frequency",
                    Frequency_peaks(fimax, timax),
                    " Hz (indices ",
                    timax,
                    " and ",
                    Indeces_peaks(fimax, timax),
                    " respectively).\n",
                )
            ...
            for tn in range(timax + 1, tn2 + 1):
                idr[tn] = np.argmin(
                    np.abs(Indeces_peaks[0 : Number_peaks[tn], tn] - idr[tn - 1])
                )
            ...
            lid = np.ravel_multi_index(
                idr[tn1:tn2], np.arange(tn1, tn2 + 1), np.shape(Frequency_peaks)
            )
        ...
        if len(self.pars) == 1:
            ...
            if self.PathOpt.lower() == "on":
                idr = self.pathopt(
                    Number_peaks, Indeces_peaks, Frequency_peaks, Wp, logw1, logw2, freq
                )  # check
            else:
                [idr, timax, fimax] = self.onestepopt(
                    Number_peaks, Indeces_peaks, Frequency_peaks, Wp, logw1, logw2, freq
                )
            lid = np.ravel_multi_index(
                [idr[tn1 : tn2 + 1], np.arange(tn1, tn2 + 1)],
        ...
        if len(self.pars) == 2 and self.method == 2:
            ...
            CL = len(ss1)
            mv[1] = (
                ss1[np.int(self.Round(0.75 * CL)) - 1]
                - ss1[np.int(self.Round(0.25 * CL)) - 1]
            )
            ss2 = np.sort(pf)
            CL = len(ss2)
            mv[3] = (
                ss2[np.int(self.Round(0.75 * CL)) - 1]
                - ss2[np.int(self.Round(0.25 * CL)) - 1]
            )
            ...
            while rdiff != 0:
                ...
                if self.PathOpt.lower() == "on":
                    ...
                    idr, timax, fimax = self.onestepopt(
                        Number_peaks, Indeces_peaks, Frequency_peaks, Wp, freq
                    )  # check
                ...
                rdiff = len(
                    np.ravel_multi_index(
                        np.nonzero(
                            p_index[tn1 : tn2 + 1] - p_index0[tn1 : tn2 + 1] != 0
                        ),
                        np.shape(p_index),
                        order="F",
                    )
                ...
                CL = len(ss1)
                mv[1] = (
                    ss1[int(self.Round(0.75 * CL)) - 1]
                    - ss1[int(self.Round(0.25 * CL)) - 1]
                )
                ss2 = np.sort(pf)
                CL = len(ss2)
                mv[3] = (
                    ss2[int(self.Round(0.75 * CL)) - 1]
                    - ss2[int(self.Round(0.25 * CL)) - 1]
                )
                ...
                if itn > self.MaxIter - 1 and rdiff != 0:
                    if self.DispMode.lower() != "off":
                        if self.DispMode.lower() != "notify":
                            print("\n")
                        print(
                            "WARNING! Did not fully converge in %d iterations (current "
                            "MaxIter"
                ...
                if gg == 0:
                    if (
                        self.DispMode.lower() != "off"
                        and self.DispMode.lower() != "notify"
                    ):
        ...
        for tn in range(tn1, tn2 + 1):
            ...
            if cpeak < NF - 1:
                ...
                if np.any(ind_aid):
                    iup = (
                        cpeak
                        + np.ravel_multi_index(ind_aid, np.shape(cs), order="F")[0]
                        + 1
                    )
                del ind_aid
            if cpeak > 2:
        ...
        if self.DispMode.lower() != "off" and self.DispMode.lower() != "notify":
            print(
                "Curve extracted: ridge frequency ",
                np.mean(tfsupp[0, :]),
                "+-",
                np.std(tfsupp[0, :]),
                " Hz, lower support ",
    ...
    def pathopt(
        self, Number_peaks, Indeces_peaks, Frequency_peaks, Wp_inst, logw1, logw2, freq
    ):
        ...
        NF = len(freq)
        tn1 = np.ravel_multi_index(
            np.nonzero(Number_peaks > 0), np.shape(Number_peaks)
        )[0]
        tn1 = tn1.astype("int")
        tn2 = np.ravel_multi_index(
            np.nonzero(Number_peaks > 0), np.shape(Number_peaks)
        )[-1]
        tn2 = tn2.astype("int")
        if np.amin(freq) > 0 and np.std(np.diff(freq, 1, 0)) > np.std(
            np.diff(np.log(freq), 1, 0)
        ):
        ...
        if isinstance(logw1, types.LambdaType):
            for tn in range(tn1 + 1, tn2 + 1):
                cf = np.reshape(
                    Frequency_peaks[0 : Number_peaks[0, tn], tn],
                    (len(Frequency_peaks[0 : Number_peaks[0, tn], tn]), 1),
                ) @ np.ones((1, Number_peaks[0, tn - 1])) - np.ones(
                    (Number_peaks[0, tn], 1)
                ) @ np.reshape(
                    Frequency_peaks[0 : Number_peaks[0, tn - 1], tn - 1],
                    (1, len(Frequency_peaks[0 : Number_peaks[0, tn - 1], tn - 1])),
                )
                CW1 = logw1(cf)
                aid_matrix = (
                    np.reshape(
                        W2[0 : Number_peaks[0, tn], tn],
                        (len(W2[0 : Number_peaks[0, tn], tn]), 1),
                    )
                    @ np.ones((1, Number_peaks[0, tn - 1]))
                    + CW1
                    + np.ones((Number_peaks[0, tn], 1))
                    @ np.reshape(
                        U[0 : Number_peaks[0, tn - 1], tn - 1],
                        (1, len(U[0 : Number_peaks[0, tn - 1], tn - 1])),
                    )
                )
                q[0 : Number_peaks[0, tn], tn] = np.argmax(aid_matrix, 1)
                U[0 : Number_peaks[0, tn], tn] = np.amax(
                    aid_matrix, 1
                )  # max along  rows
                del aid_matrix
        else:
            for tn in range(tn1 + 1, tn2 + 1):
                ci = (
                    Indeces_peaks[0 : Number_peaks[tn] + 1, tn]
                    * np.ones((1, Number_peaks[tn - 1]))
                    - np.ones((Number_peaks[tn], 1))
                    * Indeces_peaks[0 : Number_peaks[tn - 1] + 1, tn - 1].T
                )
                ci = ci + NF
                cm = ci - np.floor(ci)
                ci = np.floor(ci)
                ...
                [
                    U[0 : Number_peaks[tn] + 1, tn],
                    q[0 : Number_peaks[tn] + 1, tn],
                ] = np.amax(
                    W2[0 : Number_peaks[tn] + 1, tn]
                    * np.ones((1, Number_peaks[tn - 1]))
                    + CW1
                    + np.ones((Number_peaks[tn], 1))
                    * U[0 : Number_peaks[tn - 1] + 1, tn - 1].T,
                    1,
                )  # max along  rows
        # sanity check
        q = q.astype("int")
    ...
    def onestepopt(
        ...
        if isinstance(logw1, types.LambdaType):
            for tn in range(timax + 1, tn2 + 1):
                cf = (
                    Frequency_peaks[1 : Number_peaks[tn], tn]
                    - Frequency_peaks[idid[tn - 1], tn - 1]
                )
                CW1 = logw1(cf)
                idid[tn] = np.unravel_index(
                    np.argmax(W2[0 : Number_peaks[tn] + 1, tn] + CW1), np.shape(W2), "F"
                )[1]
            for tn in range(timax - 1, tn1 - 1, -1):
                cf = (
                    -Frequency_peaks[0 : Number_peaks[tn] + 1, tn]
                    + Frequency_peaks[idid[tn + 1], tn + 1]
                )
                CW1 = logw1(cf)
                idid[tn] = np.unravel_index(
    ...
    def rectfr(self, tfsupp, TFR, freq, wopt, method=1):
        ...
        if not isinstance(wp.fwt, dict):
            ...
            if fres.lower() == "linear":
                ...
                fwt = np.interp(
                    fxi, wp.fwt["2"], wp.fwt["1"]
                )  # this is linear interpolation, not using splines #TODO
                ...
                if cind > 1 and cind < NF:
                    if cs[cind + 1] == cs[cind - 1]:
                        ...
                        cpeak2 = (
                            ...
                            - np.ravel_multi_index(
                                np.nonzero(
                                    cs[cind:0:-1] >= cs[cind + 1 : 1 : -1]
                                    and cs[cind:0:-1] > cs[cind - 1 : 0 : -1],
                                    np.shape(cs),
                                )
                            )[0]
                        )
                    ...
                    if cs[NF - 1] < cs[NF]:
                        ...
                        cpeak = (
                            ...
                            - np.ravel_multi_index(
                                np.nonzero(
                                    cs[cind - 1 : 0 : -1] > cs[cind - 2 :: -1]
                                    and cs[cind - 1 : 0 : -1] >= cs[cind:1:-1],
                                    np.shape(cs),
                                )
                            )[0]
                        )
                ...
                for tn in range(tn1, tn2 + 1):
                    ...
                    if not np.isfinite(omg):
                        if xn > idt[tn1] and xn < idt[tn2]:
                            cw = np.angle(TFR[ii, xn + 1]) - np.angle(
                                TFR[ii, xn - 1]
                    ...
                    if np.shape(cs[(np.isnan(cs)) | np.isfinite(cs) == False])[0] == 0:
                        ...
                        if np.isfinite(omg):
                            ifreq[0, tn] = (
                                -omg
                                + (1 / C)
                                * np.sum(freq[ii - 1] * cs * (2 * np.pi * fstep))
                                / casig
                            )
                        else:
                            ifreq[0, tn] = (
                                (1 / C) * np.sum(cw * cs * (2 * np.pi * fstep)) / casig
                            )
                    ...
                    if np.isfinite(cs[0]):
                        ...
                        if nflag == 0:  # %if window FT is known in analytic form
                            ...
                            if cid1 == cid2:
                                ...
                                cmax = fwt[cid1] + (fwt[cid2] - fwt[cid1]) * (
                                    ximax - fxi[cid1]
                                ) / (fxi[cid2] - fxi[cid1])
                        casig = 2 * cs[0] / cmax
                        asig[rm, tn] = casig
            ...
            if method == 0 or method == 1:
                for tn in range(tn1, tn2 + 1):
                    ...
                    if not np.any(cs[np.isnan(cs)]) or not np.isfinite(cs):
                        ...
                        if np.isfinite(D):
                            ifreq[0, tn] = (
                                (1 / D) * np.sum(freq[ii] * cs * fstep) / casig
                            )
                        else:
        ...
        for sn in range(0, np.shape(iphi)[0]):
            iphi[sn, :] = np.unwrap(
                iphi[sn, :]
            )  # should be equivalent to MATLAB inwrap
