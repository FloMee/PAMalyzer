...
class SignalProc:
    ...
    def __init__(
        self, window_width=256, incr=128, minFreqShow=0, maxFreqShow=float("inf")
    ):
    ...
    def mel_filter(
        self, filter="mel", nfilters=40, minfreq=0, maxfreq=None, normalise=True
    ):
        ...
        if filter == "mel":
            filter_points = np.linspace(
                self.convertHztoMel(minfreq), self.convertHztoMel(maxfreq), nfilters + 2
            )
        ...
        for m in range(nfilters):
            ...
            inds2 = np.where(
                (freq_points >= bins[m + 1]) & (freq_points <= bins[m + 2])
            )
            # Compute their contributions
            filterbank[inds1, m] = (freq_points[inds1] - bins[m]) / (
                bins[m + 1] - bins[m]
            )
            filterbank[inds2, m] = (bins[m + 2] - freq_points[inds2]) / (
                bins[m + 2] - bins[m + 1]
            )
    ...
    def convertToMel(
        self, filt="mel", nfilters=40, minfreq=0, maxfreq=None, normalise=True
    ):
    ...
    def SnNR(self, startSignal, startNoise):
        ...
        pS = (
            np.sum(self.data[startSignal : startSignal + self.length] ** 2)
            / self.length
        )
    ...
    def spectrogram(
        ...
        if window == "Hann":
            window_fn = self.window_hann
        elif window == "Welch":
            window_fn = self.window_welch
        elif window == "Blackman":
            window_fn = self.window_blackman
        elif window == "Parzen":
            window_fn = self.window_parzen
        elif window == "Hamming":
            window_fn = self.window_hamming
        elif window == "BlackmanHarris":
            window_fn = self.window_blackmanharris
        elif window == "Ones":
            window_fn = self.window_ones
        ...
        if sgScale == "Mel Frequency":
            self.convertToMel(
                filt="mel", nfilters=nfilters, minfreq=0, maxfreq=None, normalise=True
            )
        elif sgScale == "Bark Frequency":
            self.convertToMel(
                filt="bark", nfilters=nfilters, minfreq=0, maxfreq=None, normalise=True
            )

        return self.sg

    def window_hann(self, window_length, dtype):
        # This is the Hann window
        window = 0.5 * (
            1 - np.cos(2 * np.pi * np.arange(window_length) / (window_length - 1))
        )
        return tf.convert_to_tensor(window, dtype=dtype)

    def window_parzen(self, window_length, dtype):
        # Parzen (window_width even)
        window_length = window_length.numpy()
        n = np.arange(window_length) - 0.5 * window_length
        window = np.where(
            np.abs(n) < 0.25 * window_length,
            1
            - 6
            * (n / (0.5 * window_length)) ** 2
            * (1 - np.abs(n) / (0.5 * window_length)),
            2 * (1 - np.abs(n) / (0.5 * window_length)) ** 3,
        )
        return tf.convert_to_tensor(window, dtype=dtype)

    def window_welch(self, window_length, dtype):
        window_length = window_length.numpy()
        window = (
            1.0
            - (
                (np.arange(window_length) - 0.5 * (window_length - 1))
                / (0.5 * (window_length - 1))
            )
            ** 2
        )
    ...
    def window_hamming(self, window_length, dtype):
        window_length = window_length.numpy()
        alpha = 0.54
        beta = 1.0 - alpha
        window = alpha - beta * np.cos(
            2 * np.pi * np.arange(window_length) / (window_length - 1)
        )
        return tf.convert_to_tensor(window, dtype=dtype)

    def window_blackman(self, window_length, dtype):
        window_length = window_length.numpy()
        alpha = 0.16
        a0 = 0.5 * (1 - alpha)
        a1 = 0.5
        ...
        window = (
            a0
            - a1 * np.cos(2 * np.pi * np.arange(window_length) / (window_length - 1))
            + a2 * np.cos(4 * np.pi * np.arange(window_length) / (window_length - 1))
        )
        return tf.convert_to_tensor(window, dtype=dtype)

    def window_blackmanharris(self, window_length, dtype):
        window_length = window_length.numpy()
        a0 = 0.358375
        a1 = 0.48829
        a2 = 0.14128
        ...
        window = (
            ...
            - a1 * np.cos(2 * np.pi * np.arange(window_length) / (window_length - 1))
            + a2 * np.cos(4 * np.pi * np.arange(window_length) / (window_length - 1))
            - a3 * np.cos(6 * np.pi * np.arange(window_length) / (window_length - 1))
        )
        return tf.convert_to_tensor(window, dtype=dtype)

    def window_ones(self, window_length, dtype):
        window_length = window_length.numpy()
    ...
    def bandpassFilter(self, data=None, sampleRate=None, start=0, end=None):
        ...
        if start == 0:
            # Low pass
            taps = signal.firwin(
                ntaps, cutoff=[end / nyquist], window=("hamming"), pass_zero=True
            )
        elif end == sampleRate / 2:
            # High pass
            taps = signal.firwin(
                ntaps, cutoff=[start / nyquist], window=("hamming"), pass_zero=False
            )
        else:
            # Bandpass
            taps = signal.firwin(
    ...
    def FastButterworthBandpass(self, data, low=0, high=None):
        ...
        if filterUnstable:
            ...
            if lowPass == 0:
                ...
                sos = signal.butter(
                    30, [lowPass, highPass], btype="bandpass", output="sos"
                )
    ...
    def invertSpectrogram(self, sg, window_width=256, incr=64, nits=10, window="Hann"):
        ...
        for i in range(nits):
            ...
            est = self.spectrogram(
                window_width, incr, onesided=False, need_even=True, window=window
            )
            phase = est / np.maximum(np.max(sg) / 1e8, np.abs(est))
            sg_best = sg * phase[: len(sg)]
        invertedSgram = self.inversion_iteration(
            sg_best, incr, calculate_offset=True, set_zero_phase=False, window=window
        )
        return np.real(invertedSgram)

    def inversion_iteration(
        self, sg, incr, calculate_offset=True, set_zero_phase=True, window="Hann"
    ):
        """
        ...
        if window == "Hann":
            ...
            # Welch
            window = (
                1.0 - ((np.arange(size) - 0.5 * (size - 1)) / (0.5 * (size - 1))) ** 2
            )
        elif window == "Hamming":
            # Hamming
            alpha = 0.54
            ...
            window = (
                a0
                - a1 * np.cos(2 * np.pi * np.arange(size) / (size - 1))
    ...
    def spectral_derivative(
        self, window_width, incr, K=2, threshold=0.5, returnAll=False
    ):
        ...
        indt, indf = np.where(
            ((sdt < 0) | (sdf < 0) | (sdtf < 0) | (sdft < 0)) & (spectral_deriv < 0)
        )
    ...
    def drawFundFreq(self, seg):
        ...
        starts = (
            np.arange(len(pitch)) * pitchshape.tunit + pitchshape.tstart
        )  # in seconds
    ...
    def formants(self, ncoeff=None):
        ...
        window = 0.5 * (
            1
            - np.cos(2 * np.pi * np.arange(self.window_width) / (self.window_width - 1))
        )
    ...
    def clickSearch(self, thresh=3):
        ...
        # Frequency band
        f0 = 24000
        # lower bound needs to be rounded down
        index_f0 = -1 + math.floor(f0 / df)
        f1 = 54000
        # upper bound needs to be rounded up
        index_f1 = -1 + math.ceil(f1 / df)
        ...
        thr_spec = (np.mean(mean_spec_all) + thresh * np.std(mean_spec_all)) * np.ones(
            (np.shape(mean_spec))
        )
    ...
    def denoise(self, alg, start=None, end=None, width=None):
        ...
        if str(alg) == "Wavelets":
            ...
            self.data = self.bandpassFilter(
                self.data, self.sampleRate, start=start, end=end
            )
        elif str(alg) == "Butterworth Bandpass":
            self.data = self.ButterworthBandpass(
                self.data, self.sampleRate, low=start, high=end
            )
        else:
            # Median Filter
    ...
    def impulse_cal(self, fs, engp=90, fp=0.75, blocksize=10):
        ...
        # Window length of 1/250 sec selected experimentally
        w1 = np.floor(fs / 250)
        ...
        # note thr - 90% for energy percentile
        ep = np.percentile(sg, engp, axis=0)
    ...
    def generateFeaturesCNN(
        self, seglen, real_spec_width, frame_size, frame_hop=None, CNNfRange=None
    ):
        """
