...
class WaveletFunctions:
    ...
    def WaveletPacket(self,
        ...
        if len(self.data) > 910 * 16000 and antialias:
            print(
                "ERROR: processing files larger than 15 min in slow antialiasing mode is disabled. Enable this only if you are ready to wait."
            )
            return
    ...
    def extractE(self, node, winsize, wpantialias=True):
        ...
        if node == 0:
            if wpantialias:
                print(
                    "Warning: you assumed antialias for a root node, this is probably not intended and will be reset now"
                )
            wpantialias = False
