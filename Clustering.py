...
class Clustering:
    ...
    def miniBatchKmeans(
        self, n_clusters=8, init="k-means++", max_iter=100, batch_size=25
    ):
    ...
    def cluster(
        ...
        if feature == "mfcc" and f1 != 0 and f2 != 0:
            mels = librosa.core.mel_frequencies(
                n_mels=n_mels, fmin=0.0, fmax=fs / 2, htk=False
            )
        ...
        for record in dataset:
            ...
            if feature == "mfcc":  # MFCC
                mfcc = librosa.feature.mfcc(
                    y=np.asarray(audiodata), sr=fs, n_mfcc=n_mels
                )
                if f1 != 0 and f2 != 0:
                    mfcc = mfcc[
                        ind_flow:ind_fhigh, :
                    ]  # Limit the frequency to the fixed range [f1, f2]
                ...
                we = ws.computeWaveletEnergy(
                    data=audiodata, sampleRate=fs, nlevels=nlevels, wpmode="new"
                )
                we = we.mean(axis=1)
                if weInds:
                    we = we[weInds]
        ...
        for i in range(len(predicted_labels)):
            dataset[i].insert(
                4, predicted_labels[i]
            )  # dataset format [[file1, seg1, syl1, features, predict], ...]
            ...
            for root, dirs, files in os.walk(str(dirname)):
                for file in files:
                    if file.lower().endswith(".wav") and file + ".data" in files:
                        ...
                        for segix in thisSpSegs:
                            seg = segments[segix]
                            syls = self.findSyllablesSeg(
                                os.path.join(root, file), seg, fs, denoise, minlen
                            )
                            for syl in syls:
    ...
    def findSyllablesSeg(self, file, seg, fs, denoise, minlen):
        ...
        syls = segment.medianClip(
            thr=3, medfiltersize=5, minaxislength=9, minSegment=50
        )
        if len(syls) == 0:  # Sanity check
            # Try again with lower threshold
            segment = Segment.Segmenter(sp, fs)
            syls = segment.medianClip(
                thr=2, medfiltersize=5, minaxislength=9, minSegment=50
            )
    ...
    def getClusterCenter(
        self, cluster, fs, f1, f2, feature, duration, n_mels=24, denoise=False
    ):
        """
        ...
        for record in cluster:
            ...
            for syl in record[2]:
                ...
                if feature == "mfcc":  # MFCC
                    ...
                    ws = WaveletSegment.WaveletSegment(spInfo={})
                    we = ws.computeWaveletEnergy(
                        data=audiodata, sampleRate=fs, nlevels=5, wpmode="new"
                    )
                    we = we.mean(axis=1)
                    if f1 != 0 and f2 != 0:
                        we = we[
                            ind_flow:ind_fhigh
                        ]  # Limit the frequency to a fixed range f1, f2
                    fc.append(we)
                elif feature == "chroma":
                    chroma = librosa.feature.chroma_cqt(y=audiodata, sr=fs)
                    # chroma = librosa.feature.chroma_stft(y=data, sr=fs)
    ...
    def loadFile(
        ...
        if denoise:
            WF = WaveletFunctions.WaveletFunctions(
                data=audiodata, wavelet="dmey2", maxLevel=10, samplerate=fs
            )
        ...
        for root, dirs, files in os.walk(str(dir)):
            for file in files:
                if file.lower().endswith(".wav") and file + ".data" in files:
                    ...
                    # one member :)
                    segments_len = [
                        segments[segix][1] - segments[segix][0] for segix in thisSpSegs
                    ]
                    ...
                    for seg in sortedsegments:
                        ...
                        if (
                            len(syls) == 0
                        ):  # Sanity check, when annotating syllables tight,
                            syls = [
                                [0, int((seg[1] - seg[0]) * fs)]
                        ...
                        if (
                            len(syls) == 1 and syls[0][1] - syls[0][0] < minlen_samples
                        ):  # Sanity check
                        ...
                        for s in syls:
                            ...
                            if feature == "mfcc":  # MFCC
                                mfcc = librosa.feature.mfcc(
                                    y=data, sr=fs, n_mfcc=n_mels
                                )
                                if f_1 != 0 and f_2 != 0:
                                    mfcc = mfcc[
                                        ind_flow:ind_fhigh, :
                                    ]  # Limit the frequency to the fixed range [f_1, f_2]
                                mfcc_delta = librosa.feature.delta(mfcc, mode="nearest")
                                mfcc = np.concatenate((mfcc, mfcc_delta), axis=0)
                                mfcc = scale(mfcc, axis=1)
                                ...
                                ws = WaveletSegment.WaveletSegment(spInfo={})
                                we = ws.computeWaveletEnergy(
                                    data=data, sampleRate=fs, nlevels=5, wpmode="new"
                                )
                                we = we.mean(axis=1)
                                if f_1 != 0 and f_2 != 0:
                                    we = we[
                                        ind_flow:ind_fhigh
                                    ]  # Limit the frequency to a fixed range f_1, f_2
                                f.append(we)
                            elif feature == "chroma":
                        ...
                        if not matched:
                            ...
                            )  # Shuffle the clusters to avoid bias
                            for c in range(len(clusters)):
                                f_c = clusters[c][
                                    "features"
                                ]  # features of the current class c
                                dist_c = np.zeros(
                                    (len(f_c), len(f))
                                )  # distances to the current class c
    ...
    def class_create(
        self, label, syl, features, f_low, f_high, segs, single=False, dist_method="dtw"
    ):
        """Create a new class
        ...
        for i in range(len(features)):
            ...
            for j in range(shift, len(features)):
                if dist_method == "dtw":
                    d, _ = librosa.sequence.dtw(
                        features[i], features[j], metric="euclidean"
                    )
        ...
        if np.count_nonzero(dist) > 0:
            nonzero = dist > 0
            inclass_d = np.percentile(
                dist[nonzero], 10
            )  # TODO: max? mean? a percentile?
        else:
            inclass_d = 0
        ...
        if single:
            ...
            ]  # get the features of the middle syllable
    ...
    def class_update(
        ...
        cluster["d"] = inclass_d
        cluster["f_low"] = (
            newf_low + cluster["f_low"]
        ) / 2  # not sure if this is correct
